...
from helper import (shift_crop_training_sample, crop_sample,
                    scale, BoundingBox, search_region, bgr2rgb,rgb2bgr)
...
class ALOVDataset(Dataset):
    ...
    def __init__(self, root_dir, ann_dir, transform=None, input_size=227):
        super(ALOVDataset, self).__init__()
        self.root_dir = root_dir
        self.ann_dir = ann_dir
        self.input_size = input_size
        self.transform = transform
        self.x, self.y = self._parse_data(root_dir, ann_dir)
    ...
    def _parse_data(self, root_dir, ann_dir):
        """
        ...
        x = []
        y = []
        envs = os.listdir(ann_dir)
        envs.sort(reverse=True)
        print('Parsing ALOV dataset...')
        for env in envs:
            env_videos = os.listdir(root_dir + env)
            env_videos.sort(reverse=True)
#             print("env", env_videos)
            for vid in env_videos:
                vid_src = self.root_dir + env + "/" + vid
                vid_ann = self.ann_dir + env + "/" + vid + ".ann"
#                 print(vid_ann)
...
#         y = y[:19]
        ...
        print('Total number of annotations in ALOV dataset = %d' % (len(y)))
        return x, y
...
#         sample=self.get_orig_sample(idx,1)
#         curr_sample,opts_curr=crop_sample(sample)

        # build prev sample
        prev_sample = self.get_orig_sample(idx, 0)
        prev_sample, opts_prev = crop_sample(prev_sample)
#         print("Before scale", opts_curr['search_location'].get_bb_list())
        # scale  --- can modify
#         print('cbb', curr_sample['bb'])
    ...
    def get_orig_sample(self, idx, i=1):
        ...
        """
        curr = bgr2rgb(cv2.imread(self.x[idx][i]))
        currbb = self.get_bb(self.y[idx][i])
        ann = self.y[idx][i].strip().split(' ')
        left = min(float(ann[1]), float(ann[3]), float(ann[5]), float(ann[7]))
        top = min(float(ann[2]), float(ann[4]), float(ann[6]), float(ann[8]))
        right = max(float(ann[1]), float(ann[3]), float(ann[5]), float(ann[7]))
        bottom = max(float(ann[2]), float(ann[4]),
                     float(ann[6]), float(ann[8]))
        return {'image': curr, 'bb': [left, top, right, bottom]}
    ...
    def show(self, idx, is_current=1):
        ...
        image = rgb2bgr(sample['image'])
        image = cv2.rectangle(image, ((int(sample['bb'][0])), int(sample['bb'][1])), (int(sample['bb'][2]), int(sample['bb'][3])), (0, 255, 0), 2)
        cv2.imshow('alov dataset sample: ' + str(idx), image)## can change to plt here
        cv2.waitKey(0)
    ...
    def show_sample(self, idx):
        ...
        bb = x['currbb']
#         print("Before unscale", bb)
        bbox = BoundingBox(bb[0], bb[1], bb[2], bb[3])
        bbox.unscale(x['currimg'])
        bb = bbox.get_bb_list()
#         print("After unscale", bb)
        curr_image = cv2.cvtColor(x['currimg'], cv2.COLOR_RGB2BGR)
        curr_image = cv2.rectangle(curr_image, (int(bb[0]), int(bb[1])), (int(bb[2]), int(bb[3])),
                                   (0, 255, 0), 20)
        concat_image = np.hstack((rgb2bgr(x['previmg']), curr_image))
...
class ILSVRC2014_DET_Dataset(Dataset):
    ...
    def get_bb(self, bbox_filepath):
        root = ET.parse(bbox_filepath).getroot()
        sz = [float(root.find('size').find('width').text),float(root.find('size').find('height').text)]
        bboxes = []
        for obj in root.findall('object'):
            xmin,xmax = obj.find('bndbox').find('xmin').text,obj.find('bndbox').find('xmax').text
            ymin,ymax = obj.find('bndbox').find('ymin').text,obj.find('bndbox').find('ymax').text
    ...
    def get_sample(self, idx):
        ...
        scaled_curr_obj = scale(curr_sample, opts_curr,self.sz,self.sz)
        scaled_prev_obj = scale(prev_sample, opts_prev,self.sz,self.sz)
        training_sample = {'previmg': scaled_prev_obj['image'],
                           'currimg': scaled_curr_obj['image'],
                           'currbb': scaled_curr_obj['bb']}
        return training_sample, opts_curr
    ...
    def get_orig_sample(self, idx):
        ...
        cur_rgb=bgr2rgb(curr)
        currbb = self.y[idx]
        sample = {'image': cur_rgb, 'bb': currbb}
        return sample
    ...
    def filter_ann(self, sz, ann):
        ...
        for an in ann:
            an_width = an[2]-an[0]
            an_height = an[3]-an[1]
            if an_width <= (0.66)*sz[0] and an_height <= (0.66)*sz[1] and an_width > 0 and an_height > 0:
                ans.append(an)
        return ans
...
#         folders = os.listdir(image_dir)
        ...
        images.sort()
        bboxes.sort()
        images = [os.path.join(image_dir, image)
                  for image in images]
        bboxes = [os.path.join(bbox_dir, bbox)
                  for bbox in bboxes]
        annotations = []
        ...
        print('Total number of annotations in ImageNet dataset =', len(y))
#         x = x[:2]
#         y = y[:2]
    ...
    def display_object(self, idx):
        ...
        sample = self.get_orig_sample(idx)
        image = rgb2bgr(sample['image'])
        image = cv2.rectangle(image, (int(sample['bb'][0]), int(sample['bb'][1])), (int(sample['bb'][2]), int(sample['bb'][3])),
                              (0, 255, 0), 2)
...
#         print(bb)
        ...
        bb = bbox.get_bb_list()
        curr_image = rgb2bgr(curr_image)
        curr_image = cv2.rectangle(curr_image, (int(bb[0]), int(bb[1])), (int(bb[2]), int(bb[3])),
                                   (0, 255, 0), 2)
        concat_image = np.hstack((rg2bgr(prev_image), curr_image))
